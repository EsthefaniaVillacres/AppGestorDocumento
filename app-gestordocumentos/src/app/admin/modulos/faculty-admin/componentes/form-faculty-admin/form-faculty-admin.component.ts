...
@Component({
  selector: 'app-form-faculty-admin',
  templateUrl: './form-faculty-admin.component.html',
  styleUrls: ['./form-faculty-admin.component.css']
})
export class FormFacultyAdminComponent {
  @Input() data: any
  ...
  create(row: any) {
    ...
    console.log(row);
    row.IdPerfil = 2
    this.facultyService.create(row).subscribe(result => {
      this.onClick.emit()
      this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Registro creado', life: 3000 });
    })
  }
  update(row: any) {
    this.facultyService.update(row, row.Id).subscribe(resul => {
      this.onClick.emit()
